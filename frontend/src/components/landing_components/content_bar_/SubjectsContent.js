...
export default function SubjectsContent({ selected_category, setter_subjects, setter_selected_categories }) {
    ...
    const tagPlaceholders = {
        1: "What was covered in the lecture? Write down the main ideas...",
        2: "Seminar solutions or homework notes...",
        3: "Other useful information, links, or ideas..."
    };
    ...
    const handler_note_click = async (note) => {
        try {
            ...
            setAttachments(selected_note.attachments || []);
    ...
    const handler_upload_file = async (e) => {
        ...
        try {
            const resp = await fetch(`http://localhost:5019/notes/${current_note.id}/attachments`, {
                method: 'POST',
                credentials: 'include',
                body: formData
            })
                            ...
                            <ul className="space-y-1 mb-4">
                                {attachments.length === 0 ? (
                                    ...
                                    attachments.map((att, idx) => (
                                        ...
                                        >
                                            {/* Link catre fisier (Nume + Iconita) */}
                                            <a
                                                href={att.file_url}
                                                target="_blank"
                                            ...
                                            <button
                                                onClick={async (e) => {
                                                    ...
                                                    if (window.confirm('Delete file?')) {
                                                        try {
                                                            const resp = await fetch(`http://localhost:5019/notes/attachments/${attId}`, {
                                                                method: 'DELETE',
                                                                credentials: 'include'
                                                            });
                                                            if (resp.ok) {
                                                                setAttachments(prev => prev.filter(a => (a.attachment_id || a.id) !== attId));
                                                            }
                                                        } catch (err) { console.error('Delete failed', err); }
                                                    }
                                                }}
                                                className="text-red-500 hover:bg-red-50 hover:text-red-700 p-1 rounded ml-2 transition-colors opacity-0 group-hover:opacity-100"
                    ...
                    {[
                        ...
                        >
                            <input
                                ...
                                className="rounded text-[#4E8DC4] focus:ring-[#4E8DC4] accent-[#4E8DC4] w-4 h-4 cursor-pointer"
                            />
