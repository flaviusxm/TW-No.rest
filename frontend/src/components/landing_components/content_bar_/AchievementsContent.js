            ...
            {loading ? <p className="text-center text-gray-400 mt-10">Loading badges...</p> : (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 pb-10">
                    {achievements.map(ach => (
                        <div
                            key={ach.id}
                            className={`relative bg-white p-4 rounded-xl shadow-sm border transition-all duration-300 hover:shadow-md flex flex-col items-center text-center group ${!ach.unlocked ? 'bg-gray-50 border-gray-200' : 'border-purple-100 ring-1 ring-purple-50'
                                }`}
                        >
                            <div className={`w-20 h-20 mb-2 transition-all duration-500 ${ach.unlocked ? 'filter-none scale-100' : 'grayscale opacity-40 group-hover:scale-105'
                                }`}>
                                <img
